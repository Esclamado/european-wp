...
<div class="BS_guest_author_form_container">
    <div class="BS_guest_author_main form-field">
        <p class="label">
            <label for="BS_guest_author_name"><?php _e('Author Name', 'guest-author'); ?></label>
        </p>
    ...
    <div id="BS_guest_author_the_rest">
        <div id="BS_guest_author_info-container">
            <div class="form-field">
                <p class="label">
                    <label for="BS_guest_author_url"><?php _e('Author URL', 'guest-author'); ?></label>
                </p>
                <input type="text" id="BS_guest_author_url" name="BS_guest_author_url" value="<?php echo esc_attr( $url ); ?>">
            </div>
            <div class="form-field">
                <p class="label">
                    <label for="BS_guest_author_description"><?php _e('Author Description', 'guest-author'); ?></label>
                </p>
        ...
        <div id="BS_guest_author_image-container">
            <div class="form-field">
                <p class="label">
                    <label for="BS_guest_author_image_url"><?php _e('Author Image', 'guest-author'); ?></label>
                </p>
                <div id="BS_guest_author_image_media_manager">
                    <div id="BS_guest_author_image_media_manager_edit_panel">
                        <p id="BS_guest_author_image_media_manager_edit_button"><span class="dashicons dashicons-edit"></span><span class="screen-reader-text"><?php _e('Edit'); ?></span></p>
                        <?php if( intval( $image_id ) > 0 ) { ?>
                            <p id="BS_guest_author_image_media_manager_remove_button"><span class="dashicons dashicons-no-alt"></span><span class="screen-reader-text"><?php _e('Remove'); ?></span></p>
                        <?php } ?>
                    </div>
                    <div id="BS_guest_author_image_media_manager_image" data-default-image="<?php echo plugins_url( '../img/default.jpg' , __FILE__ ) ?>">
                        <?php
                        if( intval( $image_id ) > 0 ) {
                            // Change with the image size you want to use
                            $image = wp_get_attachment_image( $image_id, 'medium', false, array( 'id' => 'BS-guest-author-preview-image' ) );

                            echo $image;
                        } else {
                            ?>
                            <img src="<?php echo plugins_url( '../img/default.jpg' , __FILE__ ) ?>" id="BS-guest-author-preview-image">
                            <?php
                        }
                        ?>
                    </div>
